// rikishi.js

/* To make this, enable "One Column" option in SumoDB, copy & paste the tables
 * as plain text and then turn them into array like this.
 */
export const theSekitori = [
  "Y1e Terunofuji 2-5-8",
  "O1e Kirishima 5-10",
  "O1w Hoshoryu 11-4",
  "O2e Takakeisho 8-6-1",
  "O2w Kotonowaka 10-5",
  "S1e Daieisho 6-9",
  "S1w Wakamotoharu 9-6",
  "K1e Abi 9-6",
  "K1w Nishikigi 3-12",
  "M1e Ura 6-9",
  "M1w Asanoyama 9-6",
  "M2e Atamifuji 8-7",
  "M2w Meisei 6-9",
  "M3e Oho 7-8",
  "M3w Takanosho 5-10",
  "M4e Tobizaru 8-7",
  "M4w Hiradoumi 9-6",
  "M5e Midorifuji 7-8",
  "M5w Onosato 11-4",
  "M6e Tsurugisho 2-3-10",
  "M6w Gonoyama 10-5",
  "M7e Kinbozan 6-7-2",
  "M7w Tamawashi 7-8",
  "M8e Onosho 9-6",
  "M8w Takayasu 11-4",
  "M9e Hokutofuji 6-9",
  "M9w Kotoshoho 8-7",
  "M10e Shodai 8-7",
  "M10w Mitakeumi 9-6",
  "M11e Ichiyamamoto 7-8",
  "M11w Sadanoumi 8-7",
  "M12e Shonannoumi 9-6",
  "M12w Shimazuumi 0-4-11",
  "M13e Ryuden 6-9",
  "M13w Churanoumi 7-8",
  "M14e Nishikifuji 8-7",
  "M14w Kitanowaka 3-12",
  "M15e Myogiryu 6-9",
  "M15w Roga 7-8",
  "M16e Endo 5-10",
  "M16w Daiamami 7-8",
  "M17e Takerufuji 13-2 Y",
  "J1e Tokihayate 8-7",
  "J1w Takarafuji 8-7",
  "J2e Tohakuryu 6-9",
  "J2w Mitoryu 12-3 Y",
  "J3e Hokuseiho 0-0",
  "J3w Tomokaze 9-6",
  "J4e Kagayaki 7-8",
  "J4w Oshoma 11-4",
  "J5e Bushozan 9-6",
  "J5w Shirokuma 7-6-2",
  "J6e Hakuyozan 4-11",
  "J6w Chiyoshoma 9-6",
  "J7e Hidenoumi 7-8",
  "J7w Daishoho 10-5",
  "J8e Asakoryu 9-6",
  "J8w Shishi 8-7",
  "J9e Shimanoumi 5-10",
  "J9w Tamashoho 6-9",
  "J10e Shiden 7-8",
  "J10w Wakatakakage 9-6",
  "J11e Oshoumi 7-8",
  "J11w Aoiyama 7-8",
  "J12e Kotoeko 1-14",
  "J12w Tsushimanada 7-8",
  "J13e Akua 6-9",
  "J13w Hakuoho 8-7",
  "J14e Kitaharima 4-11",
  "J14w Chiyosakae 8-7",
  "Ms1e Chiyomaru 4-3",
  "Ms1w Tenshoho 3-5",
  "Ms2e Yuma 2-5",
  "Ms2w Onokatsu 5-2",
  "Ms3e Tochimusashi 0-0-7",
  "Ms3w Kayo 4-3",
  "Ms4e Kitadaichi 3-4",
  "Ms4w Tsukahara 5-2",
  "Ms5e Kotodaigo 0-6-1",
  "Ms5w Nabatame 4-3",
  "Ms6e Kototebakari 5-2",
  "Ms6w Hokutomaru 3-4",
  "Ms7e Satorufuji 3-4",
  "Ms7w Kotokuzan 4-3",
  "Ms8e Daishomaru 4-3",
  "Ms8w Kiryuko 5-2",
  "Ms9e Dewanoryu 2-5",
  "Ms9w Tokunomusashi 4-3",
  "Ms10e Wakaikari 4-3",
  "Ms10w Hananoumi 2-4-1",
  "Ms11e Hokutenkai 3-4",
  "Ms11w Chiyonoumi 4-3",
  "Ms12e Miyagi 4-3",
  "Ms12w Mudoho 3-4",
  "Ms13e Daiseizan 6-1",
  "Ms13w Kazekeno 7-0 Y",
  "Ms14e Kotoyusho 2-4-1",
  "Ms14w Fujiseiun 4-3",
  "Ms15e Kairyu 3-4",
  "Ms15w Otsuji 5-2",
  "Ms16e Tendozan 2-5",
  "Ms16w Fujinoyama 3-4",
  "Ms17e Nishinoryu 1-6",
  "Ms17w Haruyama 5-2",
  "Ms18e Toshunryu 2-5",
  "Ms18w Oyamatoumi 3-4",
  "Ms19e Mineyaiba 4-3",
  "Ms19w Fukai 3-4",
  "Ms20e Yago 5-2",
  "Ms20w Kotokenryu 4-3",
  "Ms21e Hitoshi 5-2",
  "Ms21w Chiyotora 3-4",
  "Ms22e Osanai 1-6",
  "Ms22w Yoshii 6-1",
  "Ms23e Narutaki 4-3",
  "Ms23w Kaisho 5-2",
  "Ms24e Chiyonoo 2-5",
  "Ms24w Obara 5-2",
  "Ms25e Daikisho 2-3-2",
  "Ms25w Hatsuyama 2-5",
  "Ms26e Asagyokusei 3-4",
  "Ms26w Asanowaka 4-3",
  "Ms27e Asahakuryu 6-1",
  "Ms27w Awanokuni 2-5",
  "Ms28e Kanzaki 4-3",
  "Ms28w Tanji 4-3",
  "Ms29e Toseiryu 3-4",
  "Ms29w Kazenoumi 3-4",
  "Ms30e Hokaho 1-0-6",
  "Ms30w Fujitoshi 4-3",
  "Ms31e Kamito 3-4",
  "Ms31w Tsurubayashi 4-3",
  "Ms32e Taiga 3-4",
  "Ms32w Amakaze 1-6",
  "Ms33e Asashiyu 3-4",
  "Ms33w Tochikodai 4-3",
  "Ms34e Ryusei 4-3",
  "Ms34w Wakanosho 5-2",
  "Ms35e Tochikamiyama 0-0-7",
  "Ms35w Akinoyama 2-5",
  "Ms36e Nionoumi 2-5",
  "Ms36w Terutsuyoshi 0-0-7",
  "Ms37e Tochiseiryu 4-3",
  "Ms37w Shohoryu 3-4",
  "Ms38e Otani 1-6",
  "Ms38w Hamayutaka 5-2",
  "Ms39e Yoshiyasu 5-2",
  "Ms39w Hodaka 2-5",
  "Ms40e Asakoki 5-2",
  "Ms40w Kurohimeyama 3-4",
  "Ms41e Shiroma 4-3",
  "Ms41w Kotoozutsu 4-3",
  "Ms42e Kotodaishin 3-4",
  "Ms42w Sasakiyama 2-5",
  "Ms43e Wakatakamoto 6-1",
  "Ms43w Takakento 0-0-7",
  "Ms44e Asonoyama 5-2",
  "Ms44w Hinataryu 1-6",
  "Ms45e Shunrai 3-4",
  "Ms45w Hoshuzan 3-4",
  "Ms46e Arauma 3-4",
  "Ms46w Sazanami 4-3",
  "Ms47e Oshoryu 4-3",
  "Ms47w Kirinryu 4-3",
  "Ms48e Anosho 5-2",
  "Ms48w Kumanoryu 2-5",
  "Ms49e Suzaki 4-3",
  "Ms49w Kazuto 4-3",
  "Ms50e Seigo 3-4",
  "Ms50w Ohata 2-5",
  "Ms51e Aron 2-5",
  "Ms51w Nihonyanagi 5-2",
  "Ms52e Daishoryu 5-2",
  "Ms52w Okinohama 3-4",
  "Ms53e Denuma 3-4",
  "Ms53w Kyoda 3-4",
  "Ms54e Kaizen 4-3",
  "Ms54w Kaigo 5-2",
  "Ms55e Chiyoraizan 3-4",
  "Ms55w Kyokutaisei 6-1",
  "Ms56e Takashoki 4-3",
  "Ms56w Chiyonokatsu 3-4",
  "Ms57e Akitoba 1-6",
  "Ms57w Kotodairyu 5-2",
  "Ms58e Onojo 4-3",
  "Ms58w Kainoshima 4-3",
  "Ms59e Kaishin 2-5",
  "Ms59w Kaiseijo 4-3",
  "Ms60e Maikeru 2-5",
  "Ms60w Tosamidori 6-1",
  "Ms60TD Matsui 5-2",
  "Sd3w Nagamura 7-0 Y",
  "Sd4e Yutakasho 4-3",
  "Sd5w Oki 0-0-7",
  "Sd6w Hamanoumi 4-3",
  "Sd7e Akiyoshi 5-2",
  "Sd7w Aratakayama 1-4-2",
  "Sd8e Nobehara 4-3",
  "Sd10e Oyamada 4-3",
  "Sd10w Oka 4-3",
  "Sd11e Tochinobori 5-2",
  "Sd11w Asashinjo 4-3",
  "Sd12w Wakamiyabi 4-3",
  "Sd14e Rinko 5-2",
  "Sd16e Higonoumi 5-2",
  "Sd18e Aonishiki 6-1",
  "Sd19w Kotetsu 5-2",
  "Sd21w Tokinohira 5-2",
  "Sd25w Araoyama 5-2",
  "Sd26w Naya 6-1",
  "Sd28e Murayama 5-2",
  "Sd39e Hakuonada 6-1",
  "Sd49w Kamitani 6-1",
  "Sd50w Dewanojo 6-1",
  "Sd60w Koki 6-1",
  "Sd64w Higonomaru 6-1",
  "Sd73w Shinyashiki 6-1",
  "Sd90TD Tokitenran 6-1",
];

/* Add here the shikona of retired sekitori, who will not appear in the
 * following banzuke. If nobody retired then leave this array empty
 */
export const retiredRikishi = ["Hokuseiho", "Terutsuyoshi"];

/* Enable "No Rank Colouring" and "One Column" options and then open the
 * browser's inspector (F12). Find the table and copy & paste the <tbody> node.
 * The rikishi ID is located right after "Rikishi.aspx?r=". Turn the IDs into an
 * array (add the empty spots as 0). This array should have the same length as
 * theSekitori array.
 */
export const sekitoriID = [
  11927, 12231, 12451, 12191, 12270, 11985, 6480, 12226, 11980, 12664, 12352,
  12094, 12688, 12239, 12203, 12130, 6594, 6596, 12721, 12162, 12362, 12291,
  12646, 12314, 12210, 11946, 5944, 2879, 12113, 12453, 11855, 11784, 12320,
  12055, 12449, 12043, 12427, 12836, 11728, 12117, 12013, 11786, 12273, 12406,
  12351, 12575, 12516, 12548, 11976, 7153, 11785, 12717, 12542, 12773, 11845,
  12599, 12024, 12141, 12710, 12040, 12780, 11943, 11918, 12026, 7240, 12709,
  12165, 12634, 11736, 12674, 12774, 12370, 12342, 1241, 12114, 12793, 12255,
  12732, 12711, 12796, 12425, 12155, 12704, 12585, 12840, 12729, 12561, 12519,
  11809, 12357, 12448, 11949, 12597, 12610, 11723, 12733, 12724, 12333, 12800,
  12536, 12713, 12592, 12725, 12144, 6506, 12534, 12523, 12236, 12557, 12593,
  12767, 12319, 12699, 12075, 12727, 12771, 12220, 12484, 12576, 7156, 12199,
  12601, 11755, 12485, 12526, 2892, 12832, 12530, 12189, 12782, 12379, 11743,
  12703, 12316, 7143, 9066, 12455, 12656, 12225, 12778, 12834, 12689, 12192,
  12702,
];

export function addRikishi(basho, sekitori, sekitoriID, retiredRikishi) {
  var table1 = document.getElementById("banzuke1"),
      cell = table1.querySelectorAll(".redips-only");

  for (var i = 0; i < cell.length; i++) {
    for (var j = 0; j < sekitori.length; j++) {
      if (cell[i].classList.contains(sekitori[j].split(" ")[0])) {
        var card = document.createElement("div"),
            rikiData = sekitori[j].split(" "),
            wins = rikiData[2].split("-")[0],
            record = rikiData.length == 4 ? rikiData[2] + " " + rikiData[3] : rikiData[2];

        if (rikiData.length > 3) rikiData[2] += " " + rikiData[3];

        card.id = rikiData[0];
        card.className = "redips-drag se";
        if (rikiData[0].startsWith("Ms") || rikiData[0].startsWith("Sd") || rikiData[0].startsWith("Jd") || rikiData[0].startsWith("Jk"))
          card.setAttribute("data-w", wins * 2);
        else
          card.setAttribute("data-w", wins);
        card.setAttribute("data-re", record);

        rikiData[1] = '<a href="https://sumodb.sumogames.de/Rikishi.aspx?r=' + sekitoriID[j] + '" target="_blank">' + rikiData[1] + '</a>';
        rikiData[2] = '<a href="https://sumodb.sumogames.de/Rikishi_basho.aspx?r=' + sekitoriID[j] + "&b=" + basho + '" target="_blank">' + rikiData[2] + '</a>';

        card.innerHTML = rikiData[1];

        if (retiredRikishi.includes(sekitori[j].split(" ")[1])) {
          card.style.backgroundColor = "rgb(203, 203, 203)";
          card.className = "redips-drag intai";
          card.setAttribute("title", "Retired");
          card.removeAttribute("data-ko");
        }

        cell[i].appendChild(card);

        var resCell, newRankCell;

        if (i % 2 == 0) resCell = cell[i].previousSibling;
        else resCell = cell[i].nextSibling;

        resCell.innerHTML = rikiData[2];
      }
    }
  }
}
